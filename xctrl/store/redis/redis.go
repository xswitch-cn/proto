...
func (r *rkv) Append(record *store.Record, opts ...store.WriteOption) (err error) {
	options := store.WriteOptions{}
	options.Table = r.options.Table
	...
	rkey := fmt.Sprintf("%s%s", options.Table, record.Key)
	rkeyLock := fmt.Sprintf("%s.lock", rkey)
	res := false
	for !res {
		res, _ = r.Client.SetNX(rkeyLock, 1, 0).Result()
	}
	defer r.Client.Del(rkeyLock)
	var value interface{}
	json.Unmarshal(record.Value, &value)
	rvalueByte, err := r.Client.Get(rkey).Bytes()
	...
	if err != nil && err != redis.Nil {
		...
		err = json.Unmarshal(rvalueByte, &rvalue)
		if err != nil {
			return err
		}
		rvalue = append(rvalue, value)
	}
	rvalueByte, err = json.Marshal(rvalue)
	if err != nil {
		return err
	}
	err = r.Client.Set(rkey, rvalueByte, record.Expiry).Err()
	return err
}
